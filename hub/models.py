...
class SwapProposal(models.Model):
    ...
    proposer = models.ForeignKey(User, related_name='swap_proposals_sent', on_delete=models.CASCADE)
    receiver = models.ForeignKey(User, related_name='swap_proposals_received', on_delete=models.CASCADE)
    offered_item = models.ForeignKey(Item, related_name='offered_in_swaps', on_delete=models.CASCADE)
    requested_item = models.ForeignKey(Item, related_name='requested_in_swaps', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"Swap: {self.offered_item.name} for {self.requested_item.name}"

# Watchlist & Price Alerts
class Watchlist(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='watchlist')
    item = models.ForeignKey(Item, on_delete=models.CASCADE, related_name='watchers')
    price_threshold = models.DecimalField(max_digits=8, decimal_places=2, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.user.username} watching {self.item.name}"
...
class Report(models.Model):
    ...
    def __str__(self):
        return f"Report by {self.reporter.username} on {self.item.name}"

# College Email Verification
class CollegeDomain(models.Model):
    college_name = models.CharField(max_length=200)
    domain = models.CharField(max_length=100, unique=True)  # e.g. "student.iitb.ac.in"
    city = models.CharField(max_length=100)
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"{self.college_name} ({self.domain})"

# Item View Tracking for Analytics
class ItemView(models.Model):
    ...
    def __str__(self):
        return f"View on {self.item.name} at {self.timestamp}"

# Campus Meetup Points
